...
export const columns: ColumnDef<IData>[] = [
  ...
  },
  {
    ...
    header: ({ column }) => (
      <DataGridColumnHeader title="Created_At" column={column} />
    ),
    cell: (info) => <span>${info.getValue() as string}</span>,
    size: 120,
    meta: {
  ...
  },
  {
    ...
    header: ({ column }) => (
      <DataGridColumnHeader title="Updated_At" column={column} />
    ),
    cell: (info) => <span>{info.getValue() as string}</span>,
    size: 120,
    meta: {
...
function ActionsCell({ row }: { row: Row<IData> }) {
  ...
  const handleDelete = async (id: string[]) => {
    const url = new URL(
      `${env.NEXT_PUBLIC_SITE_URL}/api/chk7964/posts/delete-posts`
    );
